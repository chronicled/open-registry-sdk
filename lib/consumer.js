...
/**
 * Verify if an address belongs to an active Registrant
 * @function
 * @param {string} address of the ethereum account to check.
 * @returns {Promise} that resolves to a boolean indicating whether or not the registrant is active
 */
Consumer.prototype.isActiveRegistrant = function(address) {
  const invalidAddress = () => Promise.reject(new Error('Invalid address. Must be a valid hex character string with 0x prepended'));
  return this.registrar.isActiveRegistrantAsync(address, {from: this.address})
...
Consumer.prototype.getRegistrant = function(address) {
  const fetchRegistrant = address => this.registrar.registrantIndexAsync(address, {from: this.address}).then(index => this.getRegistrantbyIndex(index));
  const notFound = () => Promise.reject(new Error('The address provided does not match an active Registrant'));

  return this.isActiveRegistrant(address)
  .then(isRegistrant => isRegistrant ? fetchRegistrant(address) : notFound());
};
 ...
 */
Consumer.prototype.getRegistrantsEvents = function(fromBlock, toBlock) {
  var self = this;
  toBlock = toBlock || 'latest';
  var filter = Promise.promisifyAll(self.registrar.allEvents({fromBlock: fromBlock, toBlock: toBlock}));
  return filter.getAsync().then(function(events) {
    var filteredEvents = events.filter(function(event) {
      return event.event !== 'Error';
    });
