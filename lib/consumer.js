...
/**
 * Get Registrar contract address linked to the Registry.
 * @function
 ...
 * @function
 * @param {string} identity of the thing.
...
Consumer.prototype.getThing = function(identity) {
  ...
  return new Promise(function(fulfill, reject) {
    ...
    self.registry.getThing.call(encodedIdentity, {from: self.address}, function(err, data) {
      ...
      var builder = ProtoBuf.loadProto(schema);
...
/**
 * Get standard thing schema structure from the blockchain.
 * @function
 * @returns {Promise} that resolves with standard thing schema Object as it is on blockchain at the moment.
 */
Consumer.prototype.getStandardSchema = function() {
  var self = this;
  return new Promise(function (fulfill, reject) {
      self.registry.standardSchema.call({from: self.address}, function(error, schema) {
        if (error) {
            console.error(error);
            reject(error);
            return;
        }
        fulfill({name: schema[0], description: schema[1], definition: schema[2]});
    });
  });
};

/**
 * Get thing schema structure from the blockchain.
 * @function
 * @param {number} index of the schema starting from 0.
 * @returns {Promise} that resolves with thing schema Object as it is on blockchain at the moment.
 */
Consumer.prototype.getSchema = function(index) {
  var self = this;
  return new Promise(function (fulfill, reject) {
      self.registry.schemas.call(index, {from: self.address}, function(error, schema) {
        if (error) {
            console.error(error);
            reject(error);
            return;
...
/**
 * Get all thing schemas from the blockchain.
 * @function
...
/**
 * Verify identity against signed message. Only EC and RSA signatures are supported at the moment.
 * @function
 * @param {string} identityURN to be verified.
 * @param {string} message that has been signed.
 * @param {string} signature recieved from the thing.
