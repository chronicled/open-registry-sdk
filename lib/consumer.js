...
Consumer.prototype.getThing = function (ids) {
  var self = this;
  return new Promise(function (fulfill, reject) {
    self.registry.getThing.call(ids, {from: self.address}, function(err, data) {
      if (err) {
        console.error(err);
        reject(err);
        return;
      }
      var schema = Schema.decodeHex(data[3]);
      var dataSlices = data[1];
      var isValid = data[5];
      if (!isValid) {
        reject('Error: record marked as invalid.');
      }
      var builder = ProtoBuf.loadProto(schema.get('definition'));
