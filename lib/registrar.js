...
Registrar.prototype.addRegistrant = function(address, data) {
  ...
  return new Promise(function(fulfill, reject) {
    self.registrar.add(address, regData.encodeHex(), {from: self.address, gas: 2000000}, function(err, tx) {
      if (err) {
        console.error(err);
        reject(err);
        return;
...
Registrar.prototype.editRegistrant = function(address, data, isActive) {
  ...
  return new Promise(function(fulfill, reject) {
    self.registrar.edit(address, regData.encodeHex(), isActive, {from: self.address, gas: 2000000}, function(err, tx) {
      if (err) {
        console.error(err);
        reject(err);
        return;
