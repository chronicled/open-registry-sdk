...
//Necessary fix to ensure provider engine works for now with web3
...
const DEFAULT_GAS_LIMIT = 1400000;
...
function Provider(rpcUrl, userType, secret, contracts, params, finalCallback) {
  ...
  this.params = params || {};
  this.params.gasLimit = this.params.gasLimit || DEFAULT_GAS_LIMIT;
  this.shims = {};
  ...
  if (secret) {
    ...
    if (secret && secret.length > 70) {
      ...
      ks.init(undefined, undefined, defaultHdPathString);
      ks.defaultHdPathString = defaultHdPathString;
    ...
    engine.addProvider(new HookedWalletSubprovider({
      ...
      signTransaction: function(txData, cb) {
        // Static gas price
        var gasPrice = self.params.gasPrice || self.web3.toWei(20, 'shannon');
        txData.gasPrice = '0x' + self.web3.toBigNumber(gasPrice).toString(16);   // txData.gasPrice = parseInt(txData.gasPrice, 16);
        txData.gasLimit = '0x' + self.web3.toBigNumber(txData.gas || txData.gasLimit || self.params.gasLimit).toString(16);

        txData.nonce = parseInt(txData.nonce, 16);
        if (self.shims[txData.to]) {
          if (typeof(self.shims[txData.to]) === 'function') {
            txData = self.shims[txData.to](txData);
          }
          else {
 ...
 */
Provider.prototype.addShim = function(targetAddress, shimAddressOrFunction) {
  this.shims[targetAddress] = shimAddressOrFunction;
};
