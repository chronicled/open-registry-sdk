...
Registrant.prototype.createThing = function (dataObject, schemaIndex) {
  ...
  return new Promise(function (fulfill, reject) {
    self.registry.schemas.call(schemaIndex,{from: self.address}, function(error, proto) {
      ...
      var schema = Schema.decodeHex(proto);
      var builder = ProtoBuf.loadProto(schema.definition);
      var Thing = builder.build("Thing");
...
Registrant.prototype.createMany = function (list, schemaIndex) {
  if (!schemaIndex)
    schemaIndex = 1;
  var self = this;
  return new Promise(function (fulfill, reject) {
    self.registry.schemas.call(schemaIndex, function(error, proto) {
      ...
      var schema = Schema.decodeHex(proto);
      var builder = ProtoBuf.loadJson(ProtoBuf.DotProto.Parser.parse(schema.definition));
      var Thing = builder.build("Thing");
      var schemaIndexes = [];
      var slicesLength = [];
      var slicesArray = [];
