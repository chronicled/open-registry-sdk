...
Registrant.prototype.createThing = function(thing, schemaIndex) {
  ...
  return new Promise(function(fulfill, reject) {
    self.registry.schemas.call(schemaIndex, {from: self.address}, function(error, protoBufferSchema) {
      console.log(error, protoBufferSchema);
      if (error) {
        console.error(error);
        reject(error);
        return;
...
Registrant.prototype.createThings = function(things, schemaIndex) {
  ...
  return new Promise(function(fulfill, reject) {
    self.registry.schemas.call(schemaIndex, {from: self.address}, function(error, protoBufferSchema) {
      ...
      self.registry.createThings(idsChunks, idsPerThing, dataChunks, dataLength, schemaIndex, {from: self.address, gas: 4700000}, function(err, data) {
        if (err) {
          console.error(err);
          reject(err);
          return;
