...
Registrant.prototype.createSchema = function(schema, testThingData) {
  ...
  var self = this;
  return self.registry.standardSchemaAsync({from: self.address}).then(function(standardSchema) {
    var builder = ProtoBuf.loadProto(standardSchema[2] + schema.definition);
    var Thing = builder.build("Thing");
