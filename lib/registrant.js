...
var catcher = require('./helper').catcher;
  ...
  return catcher(function(fulfill, reject) {
    self.registry.schemas.call(schemaIndex, {from: self.address}, function(error, protoBufferSchema) {
      ...
      for (var index in things) {
        ...
        if (!OrUtils.urn.check(thing.identities)) {
          console.error('Error, identities format is invalid. Details: https://github.com/chronicled/open-registry-ethereum/#urn-specification', thing.identities);
          reject('Error, identities format is invalid.');
          return;
        }
...
function waitForTx(txHash, web3) {
  ...
  return catcher(function(resolve) {
    setTimeout(checkTX, 100, resolve);
  });
}
