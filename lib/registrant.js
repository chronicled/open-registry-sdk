...
Registrant.prototype.createAsset = function (identities, references) {
  var schemaIndex = 1;
  var self = this;
  return new Promise(function (fulfill, reject) {
    self.registry.schemas.call(schemaIndex, function(error, proto) {
      ...
      if (!references) {
        ...
        for (id in ids.identities) {
          references.push(ids.identities[id].uri);
        }
      }
      if (references.length == 0)
        reject('error, no references supplied and no ids to be used for reference');
      var slices = tools.slice(ids.encodeHex());
      self.registry.create(schemaIndex, slices, references, {from: self.address}, function(err, data) {
        if (err) {
          reject(err);
        }
        fulfill(data);
