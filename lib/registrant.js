 ...
 * @note Sends transaction, costs Ether.
 */
Registrant.prototype.createThings = function(things, schemaIndex, opts = {}) {
  ...
  return self.registry.schemasAsync(schemaIndex, {from: self.address}).then(function(protoBufferSchema) {
    ...
    return self.registry.createThingsAsync(idsChunks, idsPerThing, dataChunks, dataLength, schemaIndex, {from: self.address, gas: settings.gas});
  });
};
 ...
 * @note Sends transaction, costs Ether.
 */
Registrant.prototype.updateThingData = function(identity, data, schemaIndex, opts = {}) {
  const defaults = { gas: 4000000};
  const settings = Object.assign({}, defaults, opts);
  if (!schemaIndex){
    schemaIndex = 1;
  ...
  return self.registry.schemasAsync(schemaIndex, {from: self.address}).then(function(protoBufferSchema) {
    ...
    return self.registry.updateThingDataAsync(encodedIdentity, dataChunks, schemaIndex, {from: self.address, gas: settings.gas});
  });
};

/**
 * Delete existing Thing data on the blockchain.
 * @function
 * @param {string} identity in URN format of the Thing to be updated.
 * @returns {Promise} that resolves with transaction hash of the operation.
 * @note Sends transaction, costs Ether.
